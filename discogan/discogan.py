...
class DiscoGAN():
    def __init__(self,model_name = None) -> None:
        ...
        self.combined_model.compile(loss=['mse', 'mse', 'mae', 'mae', 'mae', 'mae'], optimizer=optimizer)
    ...
    def build_generator(self)->Model:
        ...
        def conv2d(input_layer, filters, kernel_size = 4, strides=2, padding='same', activation=LeakyReLU(alpha=0.2), normalize=True):
        ...
        c1 = conv2d(input, 64, normalize=False)
        c2 = conv2d(c1, 128)
        c3 = conv2d(c2, 256)
        c4 = conv2d(c3, 512)
        c5 = conv2d(c4, 512)
        ...
        #d1 = deconv2d(c7, c6, 512)
        d2 = deconv2d(c6, c5, 512)
        d3 = deconv2d(d2, c4, 512)
        d4 = deconv2d(d3, c3, 256)
        d5 = deconv2d(d4, c2, 128)
        d6 = deconv2d(d5, c1, 64)

        d7 = UpSampling2D(size=2)(d6)
        output = Conv2D(3, kernel_size=4, strides=1, padding='same', activation='tanh')(d7)
    ...
    def train(self, data, epochs=100, batch_size=32):
        ...
        if(self.model_name!=None):
            os.mkdir(f'./training_progression/gifs/discogan_{self.model_name}')
    ...
    def save_images(self, epoch):
        ...
        plt.savefig(f'./training_progression/gifs/discogan_{self.model_name}/image_{epoch}.png')
